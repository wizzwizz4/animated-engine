# Since I can't get pip working properly on my hackish install, I'm going to
# create a tkinter-based canvas!

import tkinter

__all__ = ['Renderer']

class TkinterRenderer:
    def __init__(self, width=640, height=480):
        self._tk = tkinter.Tk()
        self._canvas = tkinter.Canvas(master=self._tk,
                                      width=width, height=height)
        self._backimg = tkinter.BitmapImage(data="""
#define im_width 32
#define im_height 32
static char im_bits[] = {

};
""")
        self._background = self._canvas.create_image((0, 0),
                                                     anchor='nw',
                                                     image=self._backimg)
        self._canvas.pack()

Renderer = TkinterRenderer
